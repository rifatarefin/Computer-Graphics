...
point Rodrigues(point v, point a, double angle)
{
    point tmp;
    point cr=cross(a,v);
    double dot=a.x*v.x+a.y*v.y+a.z*v.z;
    tmp.x=v.x*cos(angle)+(1-cos(angle))*dot*a.x+sin(angle)*cr.x;
    tmp.y=v.y*cos(angle)+(1-cos(angle))*dot*a.y+sin(angle)*cr.y;
    tmp.z=v.z*cos(angle)+(1-cos(angle))*dot*a.z+sin(angle)*cr.z;
    return tmp;
}
